...
struct PolynomialInterpolator {
	...
	void generateCategoryName()
    {
		...
		if (order == 1)
        {
			snprintf(generatedcategoryname, sizeof(generatedcategoryname), "%d-point, 1st-order", points);
		}
        else if (order == 2)
        {
			snprintf(generatedcategoryname, sizeof(generatedcategoryname), "%d-point, 2nd-order", points);
		}
        else if (order == 3)
        {
			snprintf(generatedcategoryname, sizeof(generatedcategoryname), "%d-point, 3rd-order", points);
		}
        else
        {
			snprintf(generatedcategoryname, sizeof(generatedcategoryname), "%d-point, %dth-order", points, order);
		}
	...
	void getFullName(char *buffer)
    {
		if (pinked)
        {
			if (preemphasis)
            {
				if (oversampling == 1)
                {
					snprintf(buffer, sizeof(buffer), "Pinked, pre-emphasized, %s (%s)", name, getCategoryName());
				} else {
					snprintf(buffer, sizeof(buffer), "%dx-oversampled, pinked, pre-emphasized, %s (%s)", oversampling, name, getCategoryName());
				}
			} else {
				if (oversampling == 1) {
					snprintf(buffer, sizeof(buffer), "Pinked, %s (%s)", name, getCategoryName());
				} else {
					snprintf(buffer, sizeof(buffer), "%dx-oversampled, pinked, %s (%s)", oversampling, name, getCategoryName());
				}
			}
		} else {
			if (preemphasis) {
				if (oversampling == 1) {
					snprintf(buffer, sizeof(buffer), "Pre-emphasized, %s (%s)", name, getCategoryName());
				} else {
					snprintf(buffer, sizeof(buffer), "%dx-oversampled, pre-emphasized, %s (%s)", oversampling, name, getCategoryName());
				}
			} else {
				if (oversampling == 1) {
					snprintf(buffer, sizeof(buffer), "%s (%s)", name, getCategoryName());
				} else {
					snprintf(buffer, sizeof(buffer), "%dx-oversampled, %s (%s)", oversampling, name, getCategoryName());
				}
			}
		}
	}
...
public:
	...
	void plot(PolynomialInterpolator *ip, int differential)
    {
		...
		char buffer[200];
		snprintf(buffer, sizeof(buffer), "(%s)", ip->getCategoryName());
...
public:
	...
	void plot(PolynomialInterpolator *ip) {
		...
		if (findmax)
        {
			...
			lastmodsnr = -db;
			snprintf(buffer, sizeof(buffer), "%.1f dB", (float)db);
	...
	FrequencyResponsePlot(const char *filename, float xpi, float y12db, int xsections, int ysectionsup, int ysectionsdown) {
        ...
        {
			char buffer[200];
			snprintf(buffer, sizeof(buffer), "%d", t*12);
...
public:
	...
	PhaseRespPlot(const char *filename, float xsteppoints, float ysteppoints, float xstepfreq, float ystepphase, int xstepsleft, int xstepsright, int ystepsdown, int ystepsup) {
		...
		for (int x = -xstepsleft; (x <= xstepsright); x++)
        {
			...
			snprintf(buffer, sizeof(buffer), "%1.3f", x*xstepfreq);
		...
		for (int y = -ystepsdown; (y <= ystepsup); y++)
        {
			...
			snprintf(buffer, sizeof(buffer), "%1.3f", y*ystepphase);
	...
	void generate(floating bandbottom, floating bandtop, int allpasses1, int allpasses2, int delay) {
		...
		for(int t = 0;; t++)
        {
...

            {
				...
				PhaseRespPlot phaseplot("phase.eps", 20, 15, pi/20, pi/2/10, 0, 20, 20, 20);
				phaseplot.plot(this);
				print();
